...
describe('server SQLite database', () => {
  it('migrates an empty database, configures SQLite, and is idempotent', () => {
    ...
    // 23 before migration 013 added the outbound write queue and conflict log.
    expect(first.sqlite.prepare("SELECT count(*) AS count FROM sqlite_master WHERE type = 'table'").get()).toMatchObject({ count: 25 })
    first.close()
