...
try {
  ...
  database.sqlite.prepare("INSERT INTO people (id, name, normalized_name, color, role, sort_order, theme_id, created_at) VALUES (?, ?, ?, ?, 'child', ?, ?, ?)").run('ava', 'Ava', 'ava', '#E5484D', 0, 'minecraft', `${today}T00:00:00.000Z`)
  ...
  await page.waitForFunction(() => document.documentElement.dataset.personTheme === 'minecraft')
  await page.getByText('Ava swimming', { exact: true }).first().waitFor()
  ...
  await page.waitForFunction(() => document.documentElement.dataset.personTheme === undefined)
  await page.getByText('Leo music', { exact: true }).first().waitFor()
  if (process.env.OSL_THEME_SMOKE_ONLY !== '1') {
