...
const server = createServer(async (request, response) => {
  ...
  if (url.pathname === '/api/v1/people' && request.method === 'POST') { const body = JSON.parse(await readBody(request)); const person = { id: `person-${nextId++}`, ...body, sortOrder: people.length }; people.push(person); return json(response, 201, person) }
  ...
  const requested = url.pathname === '/' ? 'index.html' : normalize(url.pathname).replace(/^\/+/, ''); if (requested.includes('..')) return response.writeHead(403).end()
