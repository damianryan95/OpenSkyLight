...
try {
  ...
  for (const name of ['Kitchen', 'Bedroom']) { await page.getByRole('button', { name: 'Register display' }).click(); await page.getByLabel('Display name').fill(name); await page.getByRole('button', { name: 'Register', exact: true }).click() }
  ...
  if ((await page.content()).includes('must-not-appear-in-the-phone-ui')) throw new Error('a device credential appeared in the phone UI')
