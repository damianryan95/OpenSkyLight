...
function WeatherLocation() {
  const [label, setLabel] = useState(''); const [lat, setLat] = useState(''); const [lon, setLon] = useState(''); const [saved, setSaved] = useState(false); const [error, setError] = useState<string | null>(null); const [results, setResults] = useState<Array<{ label: string; lat: number; lon: number }>>([]); const [searching, setSearching] = useState(false)
  useEffect(() => { void parentGet<{ weather: { label: string; lat: number; lon: number } | null }>('/api/v1/household/settings').then((settings) => { if (settings.weather) { setLabel(settings.weather.label); setLat(String(settings.weather.lat)); setLon(String(settings.weather.lon)) } }).catch(() => undefined) }, [])
  const save = async (event: FormEvent) => { event.preventDefault(); setSaved(false); setError(null); const parsedLat = Number(lat); const parsedLon = Number(lon); if (!label.trim() || !Number.isFinite(parsedLat) || !Number.isFinite(parsedLon)) { setError('Enter a location name, latitude, and longitude.'); return }; try { await parentMutation('/api/v1/household/settings', 'PATCH', { weather: { label, lat: parsedLat, lon: parsedLon } }); setSaved(true) } catch (reason) { setError(reason instanceof ApiError ? reason.message : 'Could not save the weather location.') } }
