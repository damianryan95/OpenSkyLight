...
export function celebrationQueueReducer(state: CelebrationQueueState, action: CelebrationQueueAction): CelebrationQueueState {
  if (action.type === 'clearOverflow') return { ...state, overflowCount: 0 }
  ...
  // The overflow count has to survive the dismiss that empties the queue: that
  // is precisely when the teamwork summary is shown. Dropping it here meant the
  // count could never be rendered, however many completions had collapsed.
  if (state.pending.length === 0) return { active: null, pending: [], overflowCount: state.overflowCount }
